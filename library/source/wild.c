...
char wild[], string[];
{
   ...
   while (wild[i] || string[j]) {
      if (!cmpwild(wild,string,&i,&j)) return 0;
   }

   return 1;
}

cmpwild(wild,string,i_ptr,j_ptr)
char wild[], string[];
int  *i_ptr, *j_ptr;
{
   char f_srch, f_cmp;
   ...
   if (!f_srch) {
      if (!windex(wild,string,i,j)) return 0;
      *i_ptr=endi;
      *j_ptr=j+s_len;
      return 1;
   }
