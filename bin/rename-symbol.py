#!/usr/bin/env python3
#  vim:expandtab:sw=2:ts=2:sts=2:ai:
...
def process_file(filename, symbol_map):
  ...
  with open(filename, 'r') as in_f:
    try:
      ...
      print(f'Unable to parse {filename} due to {e}, skipping')
      return
...
def parse_args():
  p = argparse.ArgumentParser(description='Refactor ASM files to rename symbols')
  p.add_argument('--map', nargs='*', help='One or more old_symbol=new_symbol')
  p.add_argument('--mapfile', help='Mapping file containing old_symbol=new_symbol')
...
def main():
  ...
  if args.mapfile:
    with open(args.mapfile, "r") as ifp:
      for line in ifp:
        m = re.match(r'(.+)=(.+)\n', line)
        if m:
          symbol_map[m.group(1)] = m.group(2)
