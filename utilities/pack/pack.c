static char UTHwhat_string[] = "@(#) UTX-32 pack, version 1.1 of pack.c.";
/*
...
int argc; char *argv[];
{
	...
	for (k=1; k<argc; k++) {
      ...
      closein:	close (outfile);
		close (infile);
#ifdef selport
	{	time_t times[2]; 	/* SEL: st_a & st_t times not aligned */
		times[0] = status.st_atime;
		times[1] = status.st_mtime;
		utime(filename, times);/* preserve acc & mod times */
	}
#else NOT selport
