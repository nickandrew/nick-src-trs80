...
BASIC	LD	HL,VIDEO	;LOAD A BASIC TAPE.
	...
	LD	(HL),211
	INC	HL
	CALL	READ	;READ LAST 211 BYTE.
	LD	(HL),A
	INC	HL
	CALL	READ	;READ 1 CHAR PROGRAM NAME.
	LD	(HL),A
	INC	HL
TIXT	CALL	READ
	LD	(HL),A
...
GO2	CALL	READ
	LD	(HL),A
	INC	HL
LOOP2	CALL	READ	;READ CHARS UNTIL 0 (END LINE).
	LD	(HL),A
	INC	HL
	CP	0
	JR	NZ,LOOP2
...
SYSTEM	LD	HL,VIDEO	;LOAD A SYSTEM TAPE.
	...
	INC	HL
	LD	B,6
LOOP4	CALL	READ	;READ A 6 CHAR NAME.
	LD	(HL),A
	INC	HL
	DJNZ	LOOP4
LOOP6	CALL	READ	;READ KEY.
	LD	(HL),A
	INC	HL
	CP	120
	JR	Z,GETOUT
	CALL	READ	;READ BLOCK LENGTH.
	LD	(HL),A
	INC	HL
	LD	B,A
	CALL	READ	;READ LOW THEN HIGH BYTES
	LD	(HL),A	;OF BLOCK START ADDRESS.
	INC	HL
	CALL	READ
	LD	(HL),A
	INC	HL
LOOP5	CALL	READ	;READ CONTENTS OF BLOCK.
	LD	(HL),A
	INC	HL
	DJNZ	LOOP5
	CALL	READ
	...
	CALL	FLASH	;FLASH '**'
	JR	LOOP6
GETOUT	CALL	READ	;READ LOW THEN HIGH BYTES
	LD	(HL),A	;OF ENTRY ADDRESS.
	INC	HL
	CALL	READ
	LD	(HL),A
