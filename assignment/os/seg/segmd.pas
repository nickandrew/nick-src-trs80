program segmentation(input,output,disk:'osass1/out:1');
(* DISK=disk file *)
...
begin
   if qptr > 0 then
      ...
      qptr:=qptr-1
        ...
        writeln(disk,'Error. Event Queue overemptied');
        rhole.time:=0.0
        end
end;
...
begin
   ...
   if y<=low then y:=low;
   htime:= -0.5*ln(y)/ln10;
   writeln(disk,'Size ',size,' Time ',htime:8:4);
end;
...
if room then
   begin
   write(disk,'Got:  Time: ',ghole.time:8:4);
   write(disk,' Locn ',ghole.hole.where:3);
   write(disk,' Size ',ghole.hole.size:3);
   writeln(disk,' No. free blocks',fno);
   for i:=1 to fno do
      write(disk,'[',free[i].where,',',free[i].size,'] ');
   writeln(disk);
   end;
end;
...
begin
write(disk,'Rels: Time: ',rhole.time:8:4);
write(disk,' Size:',rhole.hole.size);
