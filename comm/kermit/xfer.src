...
GTCHR1	LD	A,(TEMP1)
	...
	JP	P,GTCHR2	;GO ON IF THERE IS MORE THAN ONE LEFT.
	LD	A,B		;RETURN THE COUNT IN A.
	JP	RSKP
GTCHR2	LD	(TEMP1),A
	LD	A,(CPMFLG)	;GET THE FILE TYPE FLAG
	CP	1		;ASCII
	JR	NZ,GTCHR4	;NOPE
	LD	A,(LSTCHR)	;GET LAST CHARACTER
	CP	CR		;CARRIAGE RETURN?
	JR	NZ,GTCHR4	;NOPE
	LD	A,LF		;ADD A LINE FEED
	JR	GTCR4A
GTCHR4	LD	DE,FCB
...
GTCR4A	LD	D,A		;SAVE THE CHAR.
	...
	CP	1
	JR	NZ,GTCR4B
	LD	A,D		;make sure it is a
	AND	7FH		;legal ASCII character
	LD	D,A
GTCR4B	AND	80H		;TURN OFF ALL BUT PARITY.
	LD	E,A		;SAVE THE PARITY BIT.
	LD	A,D		;RESTORE THE CHAR.
	AND	7FH		;TURN OFF THE PARITY.
	LD	(LSTCHR),A	;SAVE THIS CHARACTER 
	CP	' '		;COMPARE TO A SPACE.
	JP	M,GTCHR7	;IF LESS THEN ITS A CONTROL CHAR, HANDLE IT.
	CP	DEL		;IS THE CHAR A DELETE?
	JP	Z,GTCHR7
	CP	C		;IS IT THE QUOTE CHAR?
	JP	NZ,GTCHR8	;IF NOT PROCEED.
...
GTCHR7	LD	(TEMP2),A	;SAVE THE CHAR.
	...
	ADD	A,40H		;MAKE THE NON-CONTROL.
	AND	7FH		;MODULO 200 OCTAL.
GTCHR8	LD	HL,(CBFPTR)	;POSITION IN CHARACTER BUFFER.
	OR	E		;OR IN THE PARITY BIT.
	LD	(HL),A		;PUT THE CHAR IN THE BUFFER.
	INC	HL
	LD	(CBFPTR),HL
	INC	B		;INCREMENT THE CHAR COUNT.
...
SPACK	LD	(ARGBLK+2),A
	...
	LD	A,0		;CLEAR A (CANNOT BE XRA A, SINCE WE CAN'T TOUCH CARRY FLAG)
...
OUTPK8	LD	A,(DBFLG)
	OR	A
	JR	Z,OUTPK9
	LD	DE,SPPOS	;PRINT THE PACKET
	CALL	PRTSTR
	LD	DE,PACKET+1
	CALL	PRTSTR
OUTPK9	LD	HL,PACKET	;POINT TO THE PACKET.
