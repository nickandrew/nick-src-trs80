...
listu()
{
    ...
    for (i = 0; i < 24; i++) {
        if (ufb.uf_name[i] == 0)
            ...
            putchar(ufb.uf_name[i]);
    }
    putchar(' ');
    for (i = 0; i < 13; i++) {
        if (ufb.uf_passwd[i] == 0)
            putchar(' ');
        else
            putchar(ufb.uf_passwd[i]);
    }

    printf("%5d %5d %2d/%2d/%2d %2x %2x %2x %2x ",
           ufb.uf_uid, ufb.uf_ncalls,
           ufb.uf_lc1, ufb.uf_lc2, ufb.uf_lc3,
           ufb.uf_priv1, ufb.uf_priv2, ufb.uf_priv3, ufb.uf_tdata);
    printf("%2x %2x %2x %2x %2x %2x %2x\n",
           ufb.uf_regcount, ufb.uf_badlogin, ufb.uf_tflg1,
           ufb.uf_tflg2, ufb.uf_erase, ufb.uf_kill, ufb.uf_nothing);
}
