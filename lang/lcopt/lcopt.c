...
opt1()
{
    ...
    if (cyc[3] == 0)
        return;
    if (strcmp(line1, "\tPUSH\tHL"))
        return;
    if (!ststr(line2, "\tLD\tHL,"))
        return;
    if (strcmp(line3, "\tPOP\tDE"))
        return;
...
opt2()
{
    if (cyc[3] == 0)
        return;
    if (!ststr(line1, "\tLD\tHL,"))
        return;
    if (strcmp(line2, "\tPUSH\tHL"))
        return;
    if (strcmp(line3, line1))
        return;
    strcpy(line3, "");
    putchar('!');
    cyc[3] = 0;
