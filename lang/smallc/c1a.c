...
int     *kptr, unbound,chkglb,class;
        ...
        for (;;) {
            if (l==8) error("Declaration too complex");
                if (match("*")) {
                    left[l++]=PTR;
                } else if (match("(")) {
                    left[l++]=OPENBR;
        ...
        for (;;) {
            if (r==HIER_LEN) error("Declaration too complex");
            if (match("[")) {
                    k=needsub();
                    tarray[r++]=ARRAY;
                    arrays[ar++]=k;
                    ...
                    tarray[r++]=FUNCTION;
                    if (class==EXTERNAL || class==AUTOMATIC
                        || tarray[0]!=FUNCTION  /* as in ptr to func */
                        || realfunc) {
                        if (match(")")==0)
        ...
        if (chkglb)
            if (ptr=findglb(ssname)) {
                    if (   ptr[CLASS]!=EXTERNAL
                        && class!=EXTERNAL
                        && (ptr[IDENT]!=FUNCTION || tarray[0]!=FUNCTION)) {
        ...
        if (r==HIER_LEN) error("Declaration too complex");
        else tarray[r]=VARIABLE;

        if (tarray[0]==VARIABLE)    k=1;
        else if (tarray[0]==FUNCTION)       k=0; /* ? */
        else if (tarray[0]==POINTER)        k=1;
        else if (tarray[0]==ARRAY)  k=arrays[0];
